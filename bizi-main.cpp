...
void ejecutarEstadisticas(){
    ...
    if(obtenerEstadisticas("datos/usuarios.csv",estadisticas)){
...
void ejecutarUsuario(const unsigned numAux){
    string genero, edad;
    if(buscarUsuario("datos/usuarios.csv",numAux,genero,edad)){
        if(genero=="M"){
        ...
        cout << "El/la usuario/a " << numAux << " no aparece en el fichero \"datos/usuarios.csv\"." << endl;
    }
...
void ejecutarInforme(const string fichero, const string fraseAux){
    ...
    if(leerEstaciones("datos/estaciones.csv",estaciones)){
        if(contarUsosEstaciones(fichero,estaciones)){
            ordenarPorUso(estaciones);
            escribirInformeEstaciones(fraseAux,estaciones);
